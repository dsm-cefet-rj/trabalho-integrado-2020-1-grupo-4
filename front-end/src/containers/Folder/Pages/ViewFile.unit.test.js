...
test('button View File', () => {
    const history = createMemoryHistory();
    let file = {id: 1, nome: 'file 1'}; //verificar se existe um objeto file
    render(<Router history={history}><div>Pagina de ver File </div></Router>);
    const leftClick = { button: 0 };
    userEvent.click(screen.getByText(/yourArchive/), leftClick); //Pegar rota de pasta aberta
    expect(history.location.pathname).toBe('/projetos/1'); //pegar rota de arquivo
});